...
async function retrieveLatestIcpPrice () {
...
async function filterEvents (callerContract) {
  callerContract.events.PriceUpdatedEvent({ filter: { } }, async (err, event) => {
    ...
    console.log('* New PriceUpdated event. icpPrice: ' + event.returnValues.icpPrice)
  })
...
(async () => {
  ...
  setInterval( async () => {
    await callerContract.methods.updateIcpPrice().send({ from: ownerAddress })
  }, SLEEP_INTERVAL);
})()